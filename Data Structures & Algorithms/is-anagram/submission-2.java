class Solution {
    ...
    //    }
       ...
       {
        if(thm.containsKey(tArray[i])) 
        {
            int x = thm.get(tArray[i]);
            thm.put(tArray[i] , x+1 ) ;
        }   
         else    thm.put(tArray[i], 1);
        
       }
       for(int i=0; i<sArray.length; i++)
       {
        if(shm.containsKey(sArray[i])) shm.put(sArray[i] , shm.get(sArray[i])+1 ) ;
         else    shm.put(sArray[i], 1);
